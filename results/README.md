...
一个日志,一次 build 带出点查 + 5 档范围。**这是唯一可以直接引用的性能数据**——上表那些
pilot 汇总表都是插桩构建测的,只能当消融数据。

当前只有 `uden.data` 的四个门限跑完(off / 4 / 16 / 1e9),加上 `face.data_woff.log`——
后者**不完整**:face 的智能体结构是 root=5、1363 个叶、按大小加权每叶 24,614 键,范围查询在
哈希叶上每次要扫满 606 KB,而当时的查询数是按发射量定的 ⇒ 每轮 1.2 TB 内存流量,跑不完就被
中断了。它保留了 face 的结构与点查数字,范围那几行缺失。
